...
Date: 2020-08-07 15:14:13
LastEditors: zhoushuke
LastEditTime: 2021-04-23 11:16:35
FilePath: /fallen_deliver/utils/dingtalk_alert.py
'''
...
class DingtalkChatbot(object):
    ...
    def __init__(self, *args):
        ...
        """
        super(DingtalkChatbot, self).__init__(*args)
        self.headers = {'Content-Type': 'application/json; charset=utf-8'}
        self.queue = queue.Queue(20)  # 钉钉官方限流每分钟发送20条信息
        self.webhook = CFG['dingtalk']['api']
        self.secret = CFG['dingtalk']['secret']
        self.pc_slide = False
        self.fail_notice = False
    ...
    def update_webhook(self):
        ...
        if is_py3:
            ...
            hmac_code = hmac.new(self.secret.encode(),
                                 string_to_sign.encode(),
                                 digestmod=hashlib.sha256).digest()
        else:
            # timestamp = long(round(self.start_time * 1000))
            timestamp = int(round(self.start_time * 1000))
    ...
    def msg_open_type(self, url):
        ...
        if self.pc_slide:
            final_link = 'dingtalk://dingtalkclient/page/link?url={}&pc_slide=true'.format(
                encode_url)
        else:
            final_link = 'dingtalk://dingtalkclient/page/link?url={}&pc_slide=false'.format(
                encode_url)
        return final_link
        ...
        if all(map(is_not_null_and_blank_str, [title, text])):
            ...
            text = re.sub(
                r'(?<!!)\[.*?\]\((.*?)\)', lambda m: m.group(0).replace(
                    m.group(1), self.msg_open_type(m.group(1))), text)
            data = {
                "msgtype": "markdown",
                "markdown": {
                    "title": title,
    ...
    def send_action_card(self, action_card):
        ...
        if isinstance(action_card, ActionCard):
            ...
            logging.error("ActionCard类型：传入的实例类型不正确，内容为：{}".format(
                str(action_card)))
            raise TypeError("ActionCard类型：传入的实例类型不正确，内容为：{}".format(
                str(action_card)))
    ...
    def send_feed_card(self, links):
        ...
        for link in links:
            ...
            if isinstance(link, FeedLink) or isinstance(link, CardItem):
                ...
                raise ValueError("FeedLink类型，传入的数据格式不正确，内容为：{}".format(
                    str(link)))
    ...
    def post(self, data):
        ...
        if now - self.start_time >= 3600 and self.secret is not None and self.secret.startswith(
                'SEC'):
            self.start_time = now
            self.update_webhook()
        ...
        if self.queue.full():
            ...
            if elapse_time < 60:
                ...
                logging.debug('钉钉官方限制机器人每分钟最多发送20条，当前发送频率已达限制条件，休眠 {}s'.format(
                    str(sleep_time)))
            ...
            try:
                ...
                if self.fail_notice and result.get('errcode', True):
                    ...
                    error_data = {
                        ...
                        "text": {
                            "content":
                            "[注意-自动通知]钉钉机器人消息发送失败，时间：%s，原因：%s，请及时跟进，谢谢!" %
...
class ActionCard(object):
    ...
    def get_data(self):
        ...
        """
        if all(map(is_not_null_and_blank_str,
                   [self.title, self.text])) and len(self.btns):
            if len(self.btns) == 1:
                # 整体跳转ActionCard类型
                data = {
                    "msgtype": "actionCard",
...
class FeedLink(object):
    ...
    def get_data(self):
        ...
        """
        if all(
                map(is_not_null_and_blank_str,
                    [self.title, self.message_url, self.pic_url])):
            data = {
                "title": self.title,
...
class CardItem(object):
    ...
    def get_data(self):
        ...
        if all(
                map(is_not_null_and_blank_str,
                    [self.title, self.url, self.pic_url])):
            # FeedCard类型
            data = {
                "title": self.title,
                "messageURL": self.url,
...
if __name__ == "__main__":
    ...
    # xiaoding = DingtalkChatbot(webhook,
    #                            pc_slide=True)  # 方式三：设置消息链接在PC端侧边栏打开（v1.5以上新功能）
    # # Text消息@所有人
